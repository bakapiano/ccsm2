...
export class FitAddon implements ITerminalAddon {
  ...
  public fit(): void {
    ...
    try {
      ...
      // ResizeObserver and Dockview dimension events are asynchronous. The
      // dimension/current-size guards above already break feedback loops, so
      // keeping this locked after resize can only discard the next real layout.
  ...
  public proposeDimensions(): ITerminalDimensions | undefined {
    ...
    const paddingTop =
      Number.parseInt(elementStyle.getPropertyValue("padding-top")) || 0;
    const paddingBottom =
      Number.parseInt(elementStyle.getPropertyValue("padding-bottom")) || 0;
    const paddingLeft =
      Number.parseInt(elementStyle.getPropertyValue("padding-left")) || 0;
    const paddingRight =
      Number.parseInt(elementStyle.getPropertyValue("padding-right")) || 0;
    ...
    const cols = Math.max(
      MINIMUM_COLS,
      Math.floor(availableWidth / metrics.width),
    );
    const rows = Math.max(
      MINIMUM_ROWS,
      Math.floor(availableHeight / metrics.height),
    );
