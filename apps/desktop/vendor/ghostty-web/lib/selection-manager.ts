...
export class SelectionManager {
  ...
  // A click starts with coincident coordinates but is not a selection. Once
  // the pointer clears the drag threshold, the same coordinates intentionally
  // represent a one-cell selection.
  private selectionActive: boolean = false;
  private mouseDownX: number = 0;
  private mouseDownY: number = 0;
  // Keep the drag endpoint stable while the pointer is grazing a row edge.
  // Without hysteresis, a 1px mouse wobble at y = n * cellHeight makes the
  // endpoint alternate between the rows on every mousemove.
  ...
  getSelection(): string {
    ...
    let { col: startCol, absoluteRow: startAbsRow } = selectionStart;
    let { col: endCol, absoluteRow: endAbsRow } = selectionEnd;
    ...
    for (let absRow = startAbsRow; absRow <= endAbsRow; absRow++) {
      ...
      for (let col = colStart; col <= colEnd; col++) {
        ...
        // Ghostty represents the trailing half of a wide CJK/emoji cell as a
        // width-zero spacer. It occupies a grid column but is not text; adding
        // a space here turns "中文" into "中 文" when copied.
        if (cell.width === 0) {
          continue;
        }
  ...
  hasSelection(): boolean {
    ...
    // Programmatic selections historically use two different inclusive
    // coordinates. Mouse drags may deliberately select just one cell.
   ...
   */
  clearSelection(): void {
    const hadVisibleSelection = this.hasSelection();
  ...
  private attachEventListeners(): void {
    ...
    canvas.addEventListener('mousemove', (e: MouseEvent) => {
      if (this.isSelecting) {
        if (!this.activateDragIfThresholdMet(e.offsetX, e.offsetY)) return;

        // Mark current selection rows as dirty before updating
        this.markCurrentSelectionDirty();

        const cell = this.pixelToDragCell(e.offsetX, e.offsetY);
        const absoluteRow = this.viewportRowToAbsolute(cell.row);
        this.selectionEnd = { col: cell.col, absoluteRow };
        this.requestRender();
    ...
    this.boundDocumentMouseMoveHandler = (e: MouseEvent) => {
      if (this.isSelecting) {
        const rect = canvas.getBoundingClientRect();
        if (!this.activateDragIfThresholdMet(e.clientX - rect.left, e.clientY - rect.top)) {
          return;
        }
        ...
        if (
          ...
          if (this.autoScrollDirection === 0) {
            ...
            const cell = this.pixelToDragCell(offsetX, offsetY);
            const absoluteRow = this.viewportRowToAbsolute(cell.row);
            this.selectionEnd = { col: cell.col, absoluteRow };
            this.requestRender();
          }
    ...
    this.boundMouseUpHandler = (e: MouseEvent) => {
      if (this.isSelecting) {
        ...
        // A plain click should focus the terminal, not leave a phantom cell
        // selected. A drag within one cell remains a valid one-character range.
    ...
    this.boundContextMenuHandler = (e: MouseEvent) => {
      ...
      setTimeout(() => {
        ...
        const resetTextarea = () => {
          ...
          this.textarea.value = '';
          this.terminal.syncInputPosition(true);
  ...
  /**
   * Turn pointer motion into a real selection only after a small movement.
   * This distinguishes a focus click from dragging inside a single cell.
   */
  private activateDragIfThresholdMet(x: number, y: number): boolean {
    if (this.selectionActive) return true;
    ...
    if (!crossedDragThreshold(
      ...
      x,
      y,
      this.renderer.getMetrics().width,
    )) return false;
  ...
  /**
   * Convert a drag position to a cell while adding a small Schmitt-trigger
   * band around horizontal row boundaries. The endpoint must move a few CSS
   * pixels into the neighbouring row before it switches; crossing back needs
   * the same amount of travel in the opposite direction.
   */
  private pixelToDragCell(x: number, y: number): { col: number; row: number } {
    const cell = this.pixelToCell(x, y);
    const previousRow = this.dragRow;
    ...
    const { height } = this.renderer.getMetrics();
    const row = resolveDragRow(cell.row, previousRow, y, height);
    this.dragRow = row;
