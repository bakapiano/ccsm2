...
import { DynamicColorQueryHandler } from "./dynamic-color-query";
...
export class Terminal implements ITerminalCore {
  ...
  private lastInputCursorX: number = -1;
  private lastInputCursorY: number = -1;
  ...
  private readonly dynamicColorQueryHandler = new DynamicColorQueryHandler();
  ...
  public readonly onResize: IEvent<{ cols: number; rows: number }> =
    this.resizeEmitter.event;
  public readonly onBell: IEvent<void> = this.bellEmitter.event;
  public readonly onSelectionChange: IEvent<void> =
    this.selectionChangeEmitter.event;
  ...
  public readonly onRender: IEvent<{ start: number; end: number }> =
    this.renderEmitter.event;
  ...
  private scrollbarView?: HTMLDivElement;
    ...
    switch (key) {
      ...
      case "theme":
        if (this.renderer && this.wasmTerm) {
          ...
          if (this.compositionActive) this.positionCompositionView();
  ...
  open(parent: HTMLElement): void {
    ...
    try {
      ...
      parent.addEventListener("beforeinput", (e) => {
        // Prevent the contenteditable parent from gaining text nodes, but let
        // the real textarea receive composition updates from the platform IME.
        if (e.target === parent) e.preventDefault();
      });
      ...
      this.scrollbarView = document.createElement("div");
      this.scrollbarView.setAttribute("data-ghostty-scrollbar", "");
      this.scrollbarView.setAttribute("aria-hidden", "true");
      this.scrollbarView.setAttribute("contenteditable", "false");
      this.scrollbarView.style.position = "absolute";
      this.scrollbarView.style.top = "4px";
      this.scrollbarView.style.right = "0";
      this.scrollbarView.style.bottom = "4px";
      this.scrollbarView.style.width = `${TERMINAL_SCROLLBAR_WIDTH}px`;
      this.scrollbarView.style.opacity = "0";
      this.scrollbarView.style.pointerEvents = "none";
      this.scrollbarView.style.zIndex = "3";
      this.scrollbarThumb = document.createElement("div");
      this.scrollbarThumb.setAttribute("data-ghostty-scrollbar-thumb", "");
      this.scrollbarThumb.style.position = "absolute";
      this.scrollbarThumb.style.left = "0";
      this.scrollbarThumb.style.width = "100%";
      this.scrollbarView.appendChild(this.scrollbarThumb);
      parent.appendChild(this.scrollbarView);
      ...
      this.textarea.style.clipPath = "none";
      ...
      // Preedit text must not be written into the VT before the IME commits
      // it. Render it as a temporary DOM overlay at the Canvas cursor instead.
      ...
      this.compositionView.style.border = "0";
      this.compositionView.style.whiteSpace = "pre";
      this.compositionView.style.overflow = "hidden";
      this.compositionView.style.pointerEvents = "none";
      ...
      this.renderer = new CanvasRenderer(this.canvas, {
        ...
        opaque: !this.options.allowTransparency,
      });
      ...
      parent.addEventListener("wheel", this.handleWheel, {
        passive: false,
        capture: true,
      });
  ...
  private writeInternal(
    ...
    // Capture the user's scroll anchor before writing. A wheel gesture updates
    // targetViewportY immediately, so checking both values also covers the
    // frame between starting a smooth scroll and its first animation tick.
    const wasFollowingOutput =
      this.viewportY <= 0.01 && this.targetViewportY <= 0.01;
    const scrollbackBefore = wasFollowingOutput
      ? 0
      : this.getScrollbackLength();

    // Agent CLIs query OSC 10/11 to choose a light or dark input palette.
    // Answer from the live renderer theme before ghostty-vt sees the query;
    // the current WASM build cannot allocate these OSC responses itself.
    const colorQueries = this.dynamicColorQueryHandler.push(
      data,
      this.options.theme,
    );
    const terminalData =
      typeof data === "string"
        ? new TextDecoder().decode(colorQueries.output)
        : colorQueries.output;

    // Write directly to WASM terminal (handles VT parsing internally). A
    // standalone color query is fully consumed above and leaves no VT bytes.
    if (
      (typeof terminalData === "string" && terminalData.length > 0) ||
      (terminalData instanceof Uint8Array && terminalData.byteLength > 0)
    ) {
      this.wasmTerm!.write(terminalData);
    }

    for (const response of colorQueries.responses) {
    ...
    if (typeof terminalData === "string" && terminalData.includes("\x07")) {
      this.bellEmitter.fire();
    } else if (
      terminalData instanceof Uint8Array &&
      terminalData.includes(0x07)
    ) {
      this.bellEmitter.fire();
    }
    ...
    // Follow new output only when already at the bottom. If the user has
    // scrolled into history, keep the same absolute lines visible by moving
    // the distance-from-bottom anchor as new scrollback lines are appended.
    if (!wasFollowingOutput) {
      ...
      this.viewportY = Math.max(
        0,
        Math.min(scrollbackAfter, this.viewportY + appendedLines),
      );
      this.targetViewportY = Math.max(
        0,
        Math.min(scrollbackAfter, this.targetViewportY + appendedLines),
      );
      if (this.viewportY !== previousViewportY) {
    ...
    if (typeof terminalData === "string" && terminalData.includes("\x1b]")) {
      this.checkForTitleChange(terminalData);
    }
  ...
  reset(): void {
    ...
    // Reset in place with the VT RIS sequence. Reallocating the WASM terminal
    // here can expose uninitialized cells when the Dockview pane has changed
    // dimensions, and it also leaves SelectionManager pointing at the freed
    // GhosttyTerminal wrapper. RIS preserves object identity and clears both
    // terminal state and the visible screen through the real parser.
    this.dynamicColorQueryHandler.reset();
  ...
  /**
   * Keep the invisible textarea over the rendered cursor so the operating
   * system anchors IME preedit text and its candidate window in the right
   * place. Public for SelectionManager/context-menu restoration.
   */
  syncInputPosition(force: boolean = false): void {
    if (
      !this.isOpen ||
      !this.textarea ||
      !this.canvas ||
    ...
    const key = [anchor.left, anchor.top, anchor.width, anchor.height].join(
      ":",
    );
    ...
    this.lastInputAnchorKey = key;
    this.lastInputCursorX = cursor.x;
    this.lastInputCursorY = cursor.y;
    if (this.compositionActive) this.positionCompositionView();
  }

  /** Draw the uncommitted IME text over the Canvas without mutating VT state. */
  private positionCompositionView(): void {
    if (
    ...
    const view = this.compositionView;
    const foreground = this.options.theme.foreground ?? "#d8dee9";
    const background = this.options.theme.background ?? "#111318";
    const cursorColor = this.options.theme.cursor ?? foreground;

    view.textContent = `\u200E${this.compositionText}\u200E`;
    ...
    view.style.borderRight = `2px solid ${cursorColor}`;
    view.style.boxShadow = `inset 0 -1px ${foreground}`;
    view.style.direction = "ltr";

    // Give the native IME enough layout width for its current caret while
    // keeping the textarea itself fully transparent.
    const viewRect = view.getBoundingClientRect();
    this.textarea.style.left = `${anchor.left}px`;
    this.textarea.style.top = `${anchor.top}px`;
    this.textarea.style.width = `${Math.max(anchor.width, viewRect.width)}px`;
    this.textarea.style.height = `${anchor.height}px`;
    this.textarea.style.lineHeight = `${anchor.height}px`;
  }
  ...
  public registerLinkProvider(provider: ILinkProvider): void {
    if (!this.linkDetector) {
      throw new Error(
        "Terminal must be opened before registering link providers",
      );
    }
  ...
  public scrollLines(amount: number): void {
    ...
    const newViewportY = Math.max(
      0,
      Math.min(maxScroll, this.viewportY - amount),
    );

    if (
      newViewportY !== this.viewportY ||
      newViewportY !== this.targetViewportY
    ) {
  ...
  public scrollToBottom(): void {
    if (this.viewportY !== 0 || this.targetViewportY !== 0) {
      this.viewportY = 0;
      this.targetViewportY = 0;
      this.scrollEmitter.fire(this.viewportY);
  ...
  private startRenderLoop(): void {
    ...
    const loop = () => {
      ...
      if (this.renderActive && !this.isDisposed && this.isOpen) {
        ...
        const cursor = this.wasmTerm!.getCursor();
        if (
          cursor.x !== this.lastInputCursorX ||
          cursor.y !== this.lastInputCursorY
        ) {
  ...
  private cleanupComponents(): void {
    ...
    if (this.scrollbarView?.parentNode) {
      this.scrollbarView.parentNode.removeChild(this.scrollbarView);
    }
    this.scrollbarView = undefined;
    this.scrollbarThumb = undefined;

    // Remove IME preedit overlay from DOM
    if (this.compositionView && this.compositionView.parentNode) {
  ...
  private assertOpen(): void {
    ...
    if (!this.isOpen) {
      throw new Error(
        "Terminal must be opened before use. Call terminal.open(parent) first.",
      );
    }
  }
  ...
  private handleMouseLeave = (): void => {
    ...
    if (this.renderer && this.wasmTerm) {
      const previousHyperlinkId =
        (this.renderer as any).hoveredHyperlinkId || 0;
  ...
  private handleMouseDown = (e: MouseEvent): void => {
    ...
    const rect = this.scrollbarView.getBoundingClientRect();
    ...
    if (mouseX >= 0 && mouseX <= rect.width) {
      ...
      if (
        ...
        const scrollFraction = 1 - mouseY / rect.height; // Inverted: top = 1, bottom = 0
        const targetViewportY = Math.round(scrollFraction * scrollbackLength);
        this.scrollToLine(
          Math.max(0, Math.min(scrollbackLength, targetViewportY)),
        );
      }
    }
  };
  ...
  private processScrollbarDrag(e: MouseEvent): void {
    ...
    if (geometry.movableHeight <= 0) return;
    const scrollFraction = -deltaY / geometry.movableHeight;
  ...
  private updateScrollbarView(): void {
    if (!this.scrollbarView || !this.scrollbarThumb || !this.wasmTerm) return;
    const scrollbackLength = this.wasmTerm.getScrollbackLength();
    const trackHeight = this.scrollbarView.clientHeight;
    if (
      scrollbackLength === 0 ||
      trackHeight <= 0 ||
      this.scrollbarOpacity <= 0
    ...
    this.scrollbarView.style.opacity = String(this.scrollbarOpacity);
    this.scrollbarView.style.pointerEvents = "auto";
    this.scrollbarView.dataset.scrolled = String(this.viewportY > 0);
    this.scrollbarThumb.style.top = `${geometry.thumbTop}px`;
