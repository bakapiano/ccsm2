...
export interface RendererOptions {
  ...
  /** Use an opaque backing store. This prototype has a solid terminal background. */
  opaque?: boolean;
}
...
export interface FontMetrics {
  ...
  /** Light box-drawing stroke thickness in CSS pixels. */
  boxThickness?: number;
}
...
/** Resolve terminal cell metrics from browser measurements. Kept pure so the
 * line-box regression can be tested without depending on a host font stack. */
export function calculateFontMetrics(
  fontSize: number,
  widthMetrics: FontMetricMeasurement,
  heightMetrics: FontMetricMeasurement,
): FontMetrics {
  const width = Math.ceil(widthMetrics.width ?? fontSize * 0.6);
  const measuredAscent = Math.max(
    heightMetrics.fontBoundingBoxAscent ?? 0,
    heightMetrics.actualBoundingBoxAscent ?? 0,
  );
  const measuredDescent = Math.max(
    heightMetrics.fontBoundingBoxDescent ?? 0,
    heightMetrics.actualBoundingBoxDescent ?? 0,
  );
  const ascent = Math.ceil(measuredAscent || fontSize * 0.8);
  const descent = Math.ceil(measuredDescent || fontSize * 0.2);
...
function colorKey(color: string): number | null {
  ...
  if (!rgb) return null;
  return (
    (Number.parseInt(rgb[1], 10) << 16) |
    (Number.parseInt(rgb[2], 10) << 8) |
    Number.parseInt(rgb[3], 10)
  );
}

export function createThemeColorRemap(
  source: ITheme,
  target: ITheme,
...
export class CanvasRenderer {
  ...
  private themeColorRemap: ReadonlyMap<number, string>;
  ...
  constructor(canvas: HTMLCanvasElement, options: RendererOptions = {}) {
    this.canvas = canvas;
    const ctx = canvas.getContext("2d", { alpha: options.opaque !== true });
    if (!ctx) {
  ...
  private measureFont(): FontMetrics {
    ...
    // A single capital M has no descender and its actual ink box is much
    // shorter than the font's line box. Probe ascenders, descenders, box
    // drawing and CJK fallback, then prefer the full font bounds when the
  ...
  private resolveCellColor(r: number, g: number, b: number): string {
    return (
      this.themeColorRemap.get((r << 16) | (g << 8) | b) ??
      this.rgbToCSS(r, g, b)
    );
  }
  ...
  public render(
    ...
    const scrollbackLength = scrollbackProvider
      ? scrollbackProvider.getScrollbackLength()
      : 0;
        ...
        if (viewportY > 0) {
          if (y < viewportY && scrollbackProvider) {
            ...
            const scrollbackOffset =
              scrollbackLength - Math.floor(viewportY) + y;
  ...
  private renderCellBackground(cell: GhosttyCell, x: number, y: number): void {
    ...
    if (!isDefaultBg) {
      this.ctx.fillStyle = this.resolveCellColor(bg_r, bg_g, bg_b);
      this.ctx.fillRect(cellX, cellY, cellWidth, this.metrics.height);
    }
  }
  ...
  private renderCellText(
    ...
    if (colorOverride) {
      ...
      this.ctx.fillStyle = this.resolveCellColor(fg_r, fg_g, fg_b);
    }
    ...
    const simpleBoxOrBlock =
      cell.grapheme_len === 0 &&
      cell.codepoint > 0 &&
      isBoxOrBlock(cell.codepoint);
    const boxThickness =
      this.metrics.boxThickness ??
      Math.max(1, Math.round(this.fontSize * 0.07));
    if (
      simpleBoxOrBlock &&
      drawBoxOrBlock(
        this.ctx,
      ...
      // Box-drawing and block elements are drawn to the exact cell edges so
      // adjacent glyphs tile without font-metric gaps.
    } else {
      const char =
  ...
  private renderCursor(x: number, y: number): void {
    ...
    switch (this.cursorStyle) {
      case "block":
        ...
        // Re-draw the glyph under a block cursor with cursorAccent.
        {
