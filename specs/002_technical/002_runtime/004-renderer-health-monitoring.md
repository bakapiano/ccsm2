...
Windows首版提供独立Rust/Win32 `Reload UI`按钮。按钮使用main窗口拥有的紧凑native tool window覆盖在底部status bar最右侧，并跟随owner的可见性与z-order；main窗口visible、focused且非minimized时显示，拖动或缩放期间隐藏并在操作结束后按最终位置显示，从最小化或尺寸动画恢复时等待窗口视觉稳定后显示，因此renderer无响应时仍可点击。
