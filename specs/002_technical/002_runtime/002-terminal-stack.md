...
- Canvas renderer统一拥有CSS尺寸与整数backing尺寸；分数DPI缩放下，分配与逐帧尺寸判断使用相同的向上取整，稳定几何下保持原画布。
- 每个GhosttyTerminal在write或resize后首次读取时更新并解析一次viewport，其后逐行读取复用该快照并返回独立cell副本；scrollback读取可复用WASM临时buffer，保持JavaScript viewport快照不变。缓存仅驻留内存，持久化与PTY协议保持不变。
