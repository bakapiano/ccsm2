...
└─ supervised Agent Gateway
   ├─ TypeScript Provider adapters
   └─ Remote Web HTTPS/WebSocket
```
...
`ccsm-desktop`依赖`ccsm-core + ccsm-platform`。`ccsm-core`的dependency graph保持Tauri-free。Agent Gateway通过core定义的host protocol连接Rust；未来`ccsm-web-server`复用相同core/platform crates形成headless composition root。
...
| active Agent connection、turn/tool/approval observation   | TypeScript Agent Gateway           |
| Remote pairing、client identity和control lease            | TypeScript Agent Gateway           |
...
- Node runtime、Agent Gateway process和RuntimeReportEndpoint通过ccsm-platform接入领域层。
...
- 结构化Agent runtime通过Gateway恢复native Session；Remote Web复用同一Gateway connection和sequenced events。
