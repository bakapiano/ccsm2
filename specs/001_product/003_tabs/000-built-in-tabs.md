...
- CLI Tab将Claude Code、Codex、GitHub Copilot及Shell输出中的Space内文件引用识别为链接；`Ctrl/Cmd+Click`打开或聚焦内置File Editor Tab并跳到指定行列。HTTP/HTTPS链接通过相同手势打开同一Space内的内置Browser Tab。
